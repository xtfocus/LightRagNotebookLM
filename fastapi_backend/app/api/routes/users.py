...
async def read_waitlist_users(session: AsyncSessionDep, skip: int = 0, limit: int = 100) -> Any:
    """
    Retrieve users pending approval (waitlist).
    """
    count_statement = select(func.count()).select_from(User).where(User.is_approved == False)
    result = await session.execute(count_statement)
    count = result.scalar()

    statement = select(User).where(User.is_approved == False).offset(skip).limit(limit)
    result = await session.execute(statement)
    users = result.scalars().all()
...
async def update_user_me(
    session: AsyncSessionDep, current_user: CurrentUser, user_in: UserUpdateMe
) -> Any:
    """
    Update own user.
    """
...
async def update_password_me(
    session: AsyncSessionDep, current_user: CurrentUser, body: UpdatePassword
) -> Any:
    """
    Update own password.
    """
...
async def update_user(
    user_id: uuid.UUID, user_in: UserUpdate, session: AsyncSessionDep, current_user: CurrentUser
) -> Any:
    """
    Update a user.
    """
...
async def reject_user(
    ...
    await session.commit()
    return Message(message="User rejected and deleted successfully")
