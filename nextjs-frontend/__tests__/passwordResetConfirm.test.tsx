...
import { loginResetPassword } from "@/app/clientService";
...
describe("passwordReset action", () => {
  ...
  it("should call resetPassword with the correct input", async () => {
    ...
    (loginResetPassword as jest.Mock).mockResolvedValue({});

    await passwordResetConfirm({}, formData);

    expect(loginResetPassword).toHaveBeenCalledWith({
  ...
  it("should return an validation error if passwords are invalid and don't match", async () => {
    ...
    expect(loginResetPassword).not.toHaveBeenCalledWith();
  });

  it("should handle unexpected errors and return server error message", async () => {
    // Mock the loginResetPassword to throw an error
    const mockError = new Error("Network error");
    (loginResetPassword as jest.Mock).mockRejectedValue(mockError);
