...
describe("login action", () => {
  it("should call login service action with the correct input", async () => {
    ...
    (loginLoginAccessToken as jest.Mock).mockResolvedValue({
      data: { access_token: "1245token" },
    });

    await login({}, formData);

    expect(loginLoginAccessToken).toHaveBeenCalledWith({
      body: {
        username: "a@a.com",
        password: "Q12341414#",
      },
  ...
  it("should should return an error if either the password or username is not sent", async () => {
    ...
    expect(loginLoginAccessToken).not.toHaveBeenCalledWith();
  ...
  it("should handle unexpected errors and return server error message", async () => {
    // Mock the loginLoginAccessToken to throw an error
    const mockError = new Error("Network error");
    (loginLoginAccessToken as jest.Mock).mockRejectedValue(mockError);
