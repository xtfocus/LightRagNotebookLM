...
import { loginRecoverPassword } from "@/app/clientService";

jest.mock("../app/openapi-client/sdk.gen", () => ({
  loginRecoverPassword: jest.fn(),
}));
...
describe("passwordReset action", () => {
  ...
  it("should call resetForgotPassword with the correct input and return success message", async () => {
    ...
    (loginRecoverPassword as jest.Mock).mockResolvedValue({});

    const result = await passwordReset({}, formData);

    expect(loginRecoverPassword).toHaveBeenCalledWith({
  ...
  it("should handle unexpected errors and return server error message", async () => {
    // Mock the loginRecoverPassword to throw an error
    const mockError = new Error("Network error");
    (loginRecoverPassword as jest.Mock).mockRejectedValue(mockError);
