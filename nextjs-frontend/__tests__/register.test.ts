...
describe("register action", () => {
  it("should call register service action with the correct input", async () => {
    ...
    (usersRegisterUser as jest.Mock).mockResolvedValue({});

    await register({}, formData);

    expect(usersRegisterUser).toHaveBeenCalledWith({
      body: {
        email: "a@a.com",
        password: "Q12341414#",
      },
  ...
  it("should return an validation error if the form is invalid", async () => {
    ...
    expect(usersRegisterUser).not.toHaveBeenCalledWith();
  });

  it("should handle unexpected errors and return server error message", async () => {
    // Mock the usersRegisterUser to throw an error
    const mockError = new Error("Network error");
    (usersRegisterUser as jest.Mock).mockRejectedValue(mockError);
